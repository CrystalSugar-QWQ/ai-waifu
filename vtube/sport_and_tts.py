...
def VoiceOut_with_Emotion(question_data, message_data, tts_data, expression_data):
    ...
    while True:
        ...
        while True:
            ...
            if tts == "[end]" and message == "[end]":
                ...
                if emotion.sentiments >= 0.6:
                    expression_data.put("脸红")
                elif emotion.sentiments <= 0.2:
                    expression_data.put("脸黑")
                    
                VV.read_voice(tts)

                if emotion.sentiments >= 0.6:
                    expression_data.put("None")
                elif emotion.sentiments <= 0.2:
                    expression_data.put("None")
...
# 情绪处理，输出表情偏移量、表情动画
async def emotion_handle(websocket):
    global emotion_data
    ...
    return Brows_shifting, MouthSmile_shifting, emotion_file


async def emotion_control(websocket, expression_data):
    try:
        expression = expression_data.get_nowait()
        await vtube.expression(websocket, expression)
    except Exception:
        pass
...
async def vtube_run(expression_data):
    global is_speak
    # 连接上服务器,并初始化
    try:
        ...
        if speak_flag == 1:
            ...
            while True:
                ...
                if speak_flag == 0:
                    ...
                    await vtube.expression(websocket)
                    break
                elif speak_flag == 2:
...
def vtube_worker(expression_data):
    asyncio.run(vtube_run(expression_data))


def main(question_data, message_data):
    tts_data = queue.Queue()        # 经过翻译处理，给voicevox准备的tts数据
    message_res = queue.Queue()     # 给生成字幕准备的数据
    expression_data = queue.Queue() # 表情控制
    # tts数据翻译
    translate_thread = threading.Thread(target=translate_JA, args=(message_data, tts_data, message_res))
    translate_thread.start()

    # 语音阅读，通过全局变量告知状态
    voice_thread = threading.Thread(target=VoiceOut_with_Emotion, args=(question_data, message_res, tts_data, expression_data))
    voice_thread.start()

    # 动作输出
    VtsSport_thread = threading.Thread(target=vtube_worker, args=(expression_data, ))
